...
export default function CollectPage() {
  ...
  // Filter and sort tasks with pending first and verified last
            ...
            {paginatedTasks.map(task => (
              <div 
                ...
                className={`bg-white p-4 rounded-lg shadow-sm border ${
                  task.status === 'pending' ? 'border-yellow-300' :
                  task.status === 'in_progress' ? 'border-blue-300' :
                  task.status === 'verified' ? 'border-green-300' :
                  'border-gray-200'
                }`}
              >
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg p-6 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            {verificationStatus === 'failure' && (
              <p className="mt-2 text-blue-600 text-center text-sm">Verification failed. Please try again.</p>
            )}
...
//       try {
//         //clean response by removing markdown vode blocks if present
//         text = text.replace(/```json/g, '').replace(/```/g, '').trim()
